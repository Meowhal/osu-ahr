...
import { CommandParser, BanchoActionType } from "../CommandParser";
...
export function CommandParserTest() {
  ...
  it("player joined parse test", () => {
    ...
    if (v.type == BanchoActionType.PlayerJoined) {
  ...
  it("player left parse test", () => {
    ...
    let v = parser.ParseBanchoResponse(message);
    if (v.type == BanchoActionType.PlayerLeft) {
      assert.equal(v.id, "Swgciai");
    } else {
      assert.fail();
    }
  ...
  it("map selected test", () => {
    ...
    let v = parser.ParseBanchoResponse(message);
    if (v.type == BanchoActionType.BeatmapSelected) {
  ...
  it("host change test", ()=> {
    ...
    let v = parser.ParseBanchoResponse(message);
    if (v.type == BanchoActionType.HostChanged) {
      assert.equal(v.id, "Swgciai");
    } else {
      assert.fail();
    }
  ...
  it("match test", () => {
    ...
    assert.equal(v.type, BanchoActionType.MatchStarted);

    let message = "Swgciai finished playing (Score: 18048202, PASSED).";
    v = parser.ParseBanchoResponse(message);
    if (v.type == BanchoActionType.PlayerFinished) {
      assert.equal(v.id, "Swgciai");
    ...
    assert.equal(v.type, BanchoActionType.MatchFinished);
  });
}
