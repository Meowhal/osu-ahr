...
describe("MpSettingsParserTest", function () {
  ...
  it("mp settings parse test", () => {
    ...
    assert.equal(r.players[0].id, "gnsksz");
    assert.equal(r.players[1].id, "Discuzz");
    assert.equal(r.players[2].id, "Seidosam");
    assert.equal(r.players[3].id, "Suitaksas");
    assert.equal(r.players[4].id, "-Kasell");
  ...
  it("mp settings parse with space test", () => {
    ...
    assert.equal(r.activeMods, "Freemod");
    assert.equal(r.players.length, 5);
    assert.equal(r.players[0].id, "gns ksz");
    assert.equal(r.players[1].id, "Discuzz");
    assert.equal(r.players[2].id, "Seido sam");
    assert.equal(r.players[3].id, "Suitaksas");
    assert.equal(r.players[4].id, "-Kasell");
    assert.equal(r.players[0].isHost, true);
    assert.equal(r.players[1].isHost, false);
  ...
  it("mp settings parse with blackets test", () => {
    ...
    assert.equal(r.activeMods, "Freemod");
    assert.equal(r.players.length, 5);
    assert.equal(r.players[0].id, "gnsksz[aueie]");
    assert.equal(r.players[1].id, "Discuzz [as]v");
    assert.equal(r.players[2].id, "Sedo sam [quit]");
    assert.equal(r.players[3].id, "Suit[__]aksas");
    assert.equal(r.players[4].id, "-K][][a sell");
    assert.equal(r.players[0].isHost, true);
    assert.equal(r.players[1].isHost, false);
    assert.equal(r.players[2].isHost, false);
    assert.equal(r.players[3].isHost, false);
  ...
  it("mp settings long name (15 characters)", () => {
    ...
    assert.isTrue(p.feedLine("Slot 1  Not Ready https://osu.ppy.sh/u/x 0123456789abcde "));
    assert.isTrue(p.feedLine("Slot 2  No Map    https://osu.ppy.sh/u/x ZhiZhaChn [acv] [Hidden]"));
    assert.isTrue(p.feedLine("Slot 3  Not Ready https://osu.ppy.sh/u/1 Hot Cocoa       "));
    assert.isTrue(p.feedLine("Slot 4  Not Ready https://osu.ppy.sh/u/2 POv2II          "));
    assert.isTrue(p.feedLine("Slot 6  Not Ready https://osu.ppy.sh/u/3 MONTBLANC_heart [Host]"));
    assert.isTrue(p.feedLine("Slot 8  No Map    https://osu.ppy.sh/u/4 NewRecruit_Jack "));
    assert.isTrue(p.feedLine("Slot 9  No Map    https://osu.ppy.sh/u/5 ya nunta        "));
    assert.isTrue(p.feedLine("Slot 16 Not Ready https://osu.ppy.sh/u/6 Jow             [Hidden]"));
    assert.isFalse(p.isParsing);
    assert.isNotNull(p.result);
    ...
    assert.equal(r.players[0].id, "0123456789abcde");
    assert.equal(r.players[1].id, "ZhiZhaChn [acv]");
    assert.equal(r.players[2].id, "Hot Cocoa");
    assert.equal(r.players[3].id, "POv2II");
    assert.equal(r.players[4].id, "MONTBLANC_heart");
  ...
  it("mp settings twice", () => {
    ...
    assert.equal(r1.players[0].id, "gnsksz");
    ...
    assert.equal(r2.players[0].id, "0123456789abcde");
