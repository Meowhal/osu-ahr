...
class HostSkipper extends LobbyPlugin_1.LobbyPlugin {
    ...
    onChatCommand(player, command, param) {
        if (this.lobby.isMatching)
            return;
        if (command == "!skip") {
            if (param != "" && this.lobby.host != null && (0, Player_1.escapeUserName)(param) != this.lobby.host.escaped_name)
    ...
    StartTimer(isFirst) {
        if (this.option.afk_check_interval_ms == 0 || this.lobby.host == null || this.lobby.status != Lobby_1.LobbyStatus.Entered || this.lobby.isMatching)
            return;
        this.StopTimer();
