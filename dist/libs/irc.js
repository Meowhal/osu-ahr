...
function Client(server, clientNick, opt) {
    ...
    self.out.error = function () {
        if (!this.showDebug && !this.showErrors)
            return;
    ...
    self.addListener('raw', function (message) {
        var channels = [], channel, nick, from, text, to;
        switch (message.command) {
            ...
            case 'AUTHENTICATE':
                ...
                var authMessage = Buffer.from(self.opt.nick + '\0' +
                    self.opt.userName + '\0' +
                    self.opt.password).toString('base64');
...
Client.prototype.activateFloodProtection = function (interval) {
    var safeInterval = interval || this.opt.floodProtectionDelay, self = this;
